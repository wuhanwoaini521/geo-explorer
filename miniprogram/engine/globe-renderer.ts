...
export type GlobeVariant = "half" | "third" | "low";
...
interface RenderedMarker extends GlobeMarker {
  x: number;
  y: number;
  z: number;
}
...
export class GlobeRenderer {
  private readonly canvas: CanvasNodeLike;
  ...
  private drawDirectionalLight(): void {
    ...
    const light = ctx.createRadialGradient(
      this.centerX - this.radius * 0.48,
      this.centerY - this.radius * 0.56,
      this.radius * 0.04,
      this.centerX,
      this.centerY,
      this.radius * 1.15,
  ...
  private drawLimbDarkening(): void {
    const ctx = this.ctx;
  ...
  private drawGrid(): void {
    ...
    ctx.lineWidth = 0.45;
    [-45, 0, 45].forEach((latitude) => this.drawGeoLine(
      Array.from({ length: 25 }, (_, index) => [latitude, -180 + index * 15] as [number, number]),
    ));
    for (let longitude = -180; longitude < 180; longitude += 45) {
      this.drawGeoLine(Array.from({ length: 25 }, (_, index) => [-90 + index * 7.5, longitude] as [number, number]));
    }
  }

  private drawGeoLine(points: Array<[number, number]>): void {
    const ctx = this.ctx;
    let open = false;
  ...
  private drawMarker(marker: RenderedMarker): void {
    const ctx = this.ctx;
