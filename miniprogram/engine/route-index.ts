/**
 * 真实路线索引（V2 数据层核心）—— 纯逻辑，可在 Node 单测。
 *
 * 输入：RouteGeometryData（独立 JSON/static data，如 south-col.json）
 * 输出：RouteIndex —— 全部「距离」与「进度」都由 289 点真实折线计算得出，
 *       严禁手写示意距离（Gate 2 约束 3）。
 *
 * 三类海拔（Gate 2 约束 4）——本模块是唯一产出者，后续严禁混用：
 *   - demM   ：DEM 海拔（= z + terrain.baseM）→ 地形几何 / 路线投影
 *   - refM   ：权威参考海拔（里程碑数据，Camp/Summit 展示）
 *   - modelM ：模型海拔（pressure/O2/temp 模型）= reference-anchored（见 elevationPolicy）
 */
import type {
  RouteGeometryData,
  RouteIndex,
...
function projectOn(xs: number[], ys: number[], cum: number[], px: number, py: number) {
  const n = xs.length;
  let bestSeg = -1;
  let bestT = 0;
 ...
 */
export function buildRouteIndex(raw: RouteGeometryData): RouteIndex {
  const src = raw.points;
  if (src.length < 2) throw new Error(`[route-index] 控制点不足：${src.length}`);
  ...
  const n = xs.length;
  if (n < 2) throw new Error("[route-index] 折线退化：不足两个有效点");

  // 累计距离（水平投影，米）+ 绝对 DEM
  const cumulative = new Array<number>(n).fill(0);
  const demM = new Array<number>(n).fill(0);
  let total3d = 0;
  let ascentM = 0;
  let descentM = 0;
  for (let i = 0; i < n; i++) {
  ...
  if (!(total > MIN_SEGMENT_M)) throw new Error(`[route-index] 总长为 0：${total}`);
  ...
  const milestones: RouteMilestoneSample[] = raw.milestones
    .map((m) => {
...
function locate(index: RouteIndex, distanceM: number): { seg: number; t: number } {
  const d = clamp(distanceM, 0, index.totalDistanceM);
  let lo = 0;
  let hi = index.segmentCount - 1; // n-2
  while (lo < hi) {
    const mid = (lo + hi + 1) >> 1;
    if (index.cumulative[mid] <= d) lo = mid;
    else hi = mid - 1;
  }
  const seg = lo;
...
export function referenceElevationAt(index: RouteIndex, distanceM: number): number {
  const ms = index.milestones;
  if (ms.length === 0) return NaN;
  if (distanceM <= ms[0].distanceM) return ms[0].refM;
  if (distanceM >= ms[ms.length - 1].distanceM) return ms[ms.length - 1].refM;
  let lo = 0;
  let hi = ms.length - 1;
  while (hi - lo > 1) {
    const mid = (lo + hi) >> 1;
    if (ms[mid].distanceM <= distanceM) lo = mid;
    else hi = mid;
  }
  const a = ms[lo];
  const b = ms[hi];
...
export function routeSampleAt(index: RouteIndex, distanceM: number): RouteSampleAt {
  const d = clamp(distanceM, 0, index.totalDistanceM);
  const { seg, t } = locate(index, d);
  const segLen = index.cumulative[seg + 1] - index.cumulative[seg];
  const demElev =
    index.demM[seg] + (index.demM[seg + 1] - index.demM[seg]) * t;
  ...
  return {
    ...
    gradeDeg: ((Math.atan2(index.demM[seg + 1] - index.demM[seg], segLen) * 180) / Math.PI) as number,
  };
}

/** 按 progress（0..1）采样 */
export function routeSampleAtProgress(index: RouteIndex, progress: number): RouteSampleAt {
...
/** DEM 海拔：地形几何 / 路线投影 */
export function demElevationAt(index: RouteIndex, distanceM: number): number {
  return routeSampleAt(index, distanceM).demM;
}

/** 权威参考海拔：Camp/Summit 等 UI 展示 */
export function referenceAltitudeAt(index: RouteIndex, distanceM: number): number {
  return routeSampleAt(index, distanceM).refM;
}

/** 模型海拔：pressure / O2 / temperature 环境模型 = reference-anchored */
export function modelElevationAt(index: RouteIndex, distanceM: number): number {
