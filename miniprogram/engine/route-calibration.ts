/**
 * 校准数据 → runtime LIVE overlay（§8 / §31）。
 *
 * runtime 消费 CalibrationReport.route（运行归一化 0..1 + visibility），
 * 渲染成与 buildLiveRouteOverlay 同构的 LiveOverlayUi；
 * OCCLUDED / OUT_OF_FRAME 段不进入可见折线，起点/终点/途经点取自
 * calibration.waypoints（已验证 VISIBLE），progress 只沿可见折线插值。
 */
import type { CalibrationReportV1, ProjectedRoutePoint } from "../types/calibration";
import type {
  LiveOverlaySegmentUi,
  LiveOverlayUi,
  LiveOverlayOriginUi,
} from "./expedition-visual";
...
/** 带可见性的折线点（参与 run 判定） */
type RoutePt = Pt & { visibility: ProjectedRoutePoint["visibility"] };

/** 等步重采样（保留首末），用于控制 segment 段数 */
function decimate(pts: Pt[], maxPerRun: number): Pt[] {
  if (pts.length <= maxPerRun) return pts;
  const out: Pt[] = [];
...
function buildPolySegs(pts: Pt[]): LiveOverlaySegmentUi[] {
  const segs: LiveOverlaySegmentUi[] = [];
...
function sliceForMode(
  pts: RoutePt[],
  overlayMode: "full-route" | "nearby" | "current-next" | "none",
  localProgress: number,
): RoutePt[] {
  if (overlayMode === "none") return [];
  if (overlayMode === "full-route") return pts;
 ...
 */
export function buildCalibratedLiveOverlay(
  report: Pick<CalibrationReportV1, "route" | "waypoints"> | null | undefined,
  overlayMode: "full-route" | "nearby" | "current-next" | "none",
  localProgress: number,
): LiveOverlayUi | null {
  if (!report || !report.route || report.route.length < 2) return null;
  const raw: RoutePt[] = report.route.map((p) => ({
    u: p.u,
    v: p.v,
  ...
  for (let i = 0; i < window.length; i++) {
    ...
    cur.push(window[i]);
    if (i === window.length - 1) break;
    if (window[i + 1].visibility !== "VISIBLE") {
      flushRun();
    } else {
      const d = Math.hypot(
        window[i + 1].u - window[i].u,
        window[i + 1].v - window[i].v,
      );
      if (d > 0.5) flushRun(); // 越出/深度断层（仍可见但相距极远）
    }
  }
  flushRun();
  ...
  for (const run of runs) {
    if (run.length >= 2) segments.push(...buildPolySegs(decimate(run, 10)));
  }
  ...
  const origins: LiveOverlayOriginUi[] = (report.waypoints ?? []).map((wp) => ({
    key: wp.waypointId,
    x: wp.u * 100,
    y: wp.v * 100,
