...
interface CampFact { label: string; value: string }
...
function buildDetail(id: string, activeTab = "概览"): CampDetail {
  const point = EVEREST_EXPEDITION.routeIndex.milestones.find((m) => m.id === id) ??
    EVEREST_EXPEDITION.routeIndex.milestones.find((m) => m.id === "lhotse-face-camp-iii")!;
  const isCamp3 = point.id === "lhotse-face-camp-iii";
  const tabCopy: Record<string, { title: string; body: string }> = isCamp3
    ? {
        概览: {
...
Page({
  data: {
    tabs: TABS,
    detail: null as CampDetail | null,
  },
  ...
  onTabTap(e: PageEvent) {
    const tab = String(e.currentTarget?.dataset?.tab || "概览");
