...
class WebGLGlobeRenderer {
    constructor(canvas, width, height, pixelRatio, variant = "half", options = {}) {
        var _a, _b, _c;
        this.markers = [];
        ...
        this.options = {
            ...
            bumpScale: Math.max(0, Math.min(1.4, (_a = options.bumpScale) !== null && _a !== void 0 ? _a : 0.82)),
        ...
        if (!this.markerBuffer)
            throw new Error("WebGL marker buffer unavailable");
    ...
    getContext(canvas) {
        ...
        const context = (_a = canvas.getContext("webgl", attributes)) !== null && _a !== void 0 ? _a : canvas.getContext("experimental-webgl", attributes);
        if (!context)
    ...
    createProgram(vertexSource, fragmentSource) {
        ...
        if (!this.gl.getProgramParameter(program, GL.LINK_STATUS)) {
            throw new Error((_a = this.gl.getProgramInfoLog(program)) !== null && _a !== void 0 ? _a : "WebGL program link failed");
        }
        return program;
    }
    createShader(type, source) {
        ...
        if (!this.gl.getShaderParameter(shader, GL.COMPILE_STATUS)) {
            const log = (_a = this.gl.getShaderInfoLog(shader)) !== null && _a !== void 0 ? _a : "WebGL shader compile failed";
    ...
    createTextures() {
        const create = () => {
            ...
            if (!texture)
                throw new Error("WebGL texture unavailable");
    ...
    drawMarkerPoints() {
        if (!this.markers.length || this.markerPositionLocation < 0)
            return;
        const selected = [];
        const normal = [];
        ...
        if (normal.length)
            this.drawMarkerPointGroup(normal, 11, 0.30, 0.88, 0.94, 0.94);
        if (selected.length)
            this.drawMarkerPointGroup(selected, 22, 1.0, 0.35, 0.24, 1.0);
        this.gl.enable(GL.DEPTH_TEST);
    }
    drawMarkerPointGroup(points, size, red, green, blue, alpha) {
        this.gl.bindBuffer(GL.ARRAY_BUFFER, this.markerBuffer);
    ...
    bindTexture(index, texture, location) {
        this.gl.activeTexture(GL.TEXTURE0 + index);
        this.gl.bindTexture(GL.TEXTURE_2D, texture);
    ...
    updateProjections() {
        this.renderedMarkers = this.markers
            .map((marker) => {
            const point = (0, globe_renderer_1.projectGlobePoint)(marker.latitude, marker.longitude, this.rotation, this.pitch, this.centerX, this.centerY, this.radius);
...
exports.WebGLGlobeRenderer = WebGLGlobeRenderer;
