...
import type { CameraPose, WorldPt } from "./camera-math.js";

export interface DemGrid {
  /** 行序 north→south；列序 west→east */
  rows: number;
  cols: number;
  /** 网格原点的世界坐标（西北角；+X East / +Y South） */
  origin: WorldPt;
  /** 网格间距（米），x 与 y 取同一 */
  step: number;
  /** 高程（m AMSL? no —— 与 world.z 一致：z = elev - base） */
  z: Float32Array;
}

/** 网格索引 → (x, y, z) */
export function gridAt(grid: DemGrid, row: number, col: number): WorldPt | null {
...
export function worldToCell(grid: DemGrid, p: WorldPt): { row: number; col: number } | null {
...
export function worldToDemZ(grid: DemGrid, p: WorldPt): number | null {
...
export function demZAt(grid: DemGrid, p: WorldPt): number | null {
...
export function isOccluded(
  ...
  if (distXY < grid.step) return false;
  ...
  for (let s = 1; s < steps; s++) {
    ...
    if (distToCam < grid.step) continue;
    // 视线在该采样点的高度（修正：目标点照到远处）
    if (elev > p.z + 0.5) return true;
  }
  return false;
}

export type OcclusionStatus = "VISIBLE" | "OCCLUDED";
...
export function projectAll(
  ...
  return points.map((p) => {
    ...
    if (!cam || cam.z <= 0) return { x: 0, y: 0, ok: false };
...
function worldCameraHelper(p: WorldPt, pose: CameraPose): { x: number; y: number; z: number } | null {
