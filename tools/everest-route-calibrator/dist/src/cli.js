...
function loadPixels(path) {
    ...
    if (!Array.isArray(data))
        throw new Error("pixels file 需为数组");
    for (const p of data) {
...
function loadDem(rawPath) {
    const metaPath = rawPath.replace(/\.raw$/, ".json");
    if (!existsSync(metaPath))
        throw new Error(`缺少 DEM 元数据 ${metaPath}`);
    let meta;
    try {
...
function run(argv) {
    ...
    const result = assembleReport({
        ...
        dem,
        outDir,
        extraLimitations: dem === null ? ["未加载 DEM；遮挡全部按 VISIBLE 处理（推荐 --dem）。"] : undefined,
    });
    console.log("");
